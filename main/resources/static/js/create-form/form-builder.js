            ...
            children: [
              { tag: "span", text: "Question Type:" },
              {
                ...
                children: [
                  ...
                  { tag: "option", attributes: { value: "file" }, text: "File Upload" }
                ]
              }
            ]
          },
  ...
  document.getElementById('create-form').addEventListener('click', () => {
    ...
    questions.forEach((questionDiv, index) => {
      ...
      switch (type) {
       ...
       case 'file':
         validations.required = validationOptions.querySelector('.required')?.checked || false;
         const allowedFormatsInput = validationOptions.querySelector('.allowed-formats')?.value || '';
         const maxSizeInput = validationOptions.querySelector('.max-size')?.value || '';
         try {
           validations.allowedFormats = allowedFormatsInput
             .split(',')
             .map(format => format.trim())
             .filter(format => format !== '');
         } catch (error) {
           validations.allowedFormats = [];
         }
         validations.maxSize = maxSizeInput ? parseInt(maxSizeInput, 10) : null;
         if (isNaN(validations.maxSize)) validations.maxSize = null;
         break;
